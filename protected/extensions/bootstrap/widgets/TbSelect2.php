...
class TbSelect2 extends CInputWidget
{
	...
	// {
		...
		// if(isset($this->options['placeholder'])){
			// echo "jQuery('#{$id}').prepend('<option></option>');";
		// }
	...
	public function registerClientScript($id)
	{
		...
		$defValue = !empty($this->val) ? ".select2('val', ".CJavaScript::encode($this->val).")" : ".select2('val', '')";
		...
		// if(isset($this->val) && !empty($this->val)){
			// echo "jQuery('#{$id}').select2('val','{$this->val}');";
		// }else
			// echo "jQuery('#{$id}').select2('val','');";
