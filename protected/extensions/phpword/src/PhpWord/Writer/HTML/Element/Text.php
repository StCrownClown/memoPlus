...
class Text extends AbstractElement
{
    ...
    public function write()
    {
     ...
     /*   try
        {
                   @ $rest2 = substr($element->getText(), 2, -1);

        @$laststr .= substr($element->getText(), -1);
        @$stringtst .= $element->getText()[0].$element->getText()[1];
        }
        catch (Exception $e)
        {
        ...
        //$content .= "Before".$laststr;

        if(strpos($stringtst, '${') !== FALSE)
        {

                    if(substr($rest2,-1)=='}')

                        {
                            $test = substr($rest2, 0,-1);
                        }
                    elseif (substr($rest2,-1)=='e') {
                        {
                            $i=$i+1;
                            $test = $rest2.$i;
                        }
                        ...
                        else
                        {
                            ...
                            $content .=  '<form method="post" action="../testtemplate.php" >
                            <input type="hidden" name="rest[]" value="'. $test.'">
                             <input type="text" name="name_txt[]">';

                             //   $content .= "". $test;
            
        }
        elseif(strpos($laststr,'}')!==FALSE)
        {
