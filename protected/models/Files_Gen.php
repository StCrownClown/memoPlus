 ...
 */
class Files_Gen extends CActiveRecord
{
	...
	public function ConvertTag2Html($id) {
		...
		$pattern = "/\\#{(Time\d*)}/";
		$replacement = '<div class="input-group bootstrap-timepicker timepicker" style="width:80px;"><input id="$1" type="textbox" class="form-control input-small" name="$1" style="width:70px;"><span class="input-group-addon"><i class="glyphicon glyphicon-time"></i></span></div>';
		$html = preg_replace($pattern, $replacement, $html);
		
		$pattern = "/\\#{((Select)(\w*)(\d\d))}/";
		$replacement = '<select name="$1" id="$1" class="test">##[$3]##</select>';
		$html = preg_replace($pattern, $replacement, $html);

		$models = new TestSelect;
		$models = TestSelect::model()->findAll();
	...
	public function rules()
	{
		...
		return array(
			...
			array('Name, html', 'length', 'max'=>100),
...
//                 $criteria->order = 'Name ASC';
        ...
        public function search_lastselect($id)
	{
		...
		$criteria=new CDbCriteria;
				//$criteria->addInCondition("FilesID", $id);
				//var_dump($id);
				//die();
	 ...
	 * @return Files_Gen the static model class
	 */
